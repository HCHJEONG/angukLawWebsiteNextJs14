...
export function Level() {
  const { nodes } = useGLTF('/assets/glb/level-react-draco.glb') as any
...
export function Camera() {
  const { nodes, materials } = useGLTF('/assets/glb/level-react-draco.glb') as any
...
export function Box({ scale = 1, ...props }) {
  ...
  useFrame((state, delta) => ((ref.current as any).rotation.x = (ref.current as any).rotation.y += delta))
  return (
    <mesh
      {...props}
      ref={ref}
